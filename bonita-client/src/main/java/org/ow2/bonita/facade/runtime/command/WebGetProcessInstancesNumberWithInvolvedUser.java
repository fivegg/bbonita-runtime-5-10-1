 ...
 * 
 */
public class WebGetProcessInstancesNumberWithInvolvedUser implements Command<Integer> {
  ...
  public WebGetProcessInstancesNumberWithInvolvedUser(final String userId, final Set<String> roleUUIDs,
    ...
    switch (processStartPolicy) {
      case ALLOW_BY_DEFAULT:
        ...
        if (processUUIDs != null && !processUUIDs.isEmpty()) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithInvolvedUserExcept(username, processUUIDs);
          return result;
        } else {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithInvolvedUser(username);
          return result;
        }
      ...
      case DENY_BY_DEFAULT:
        ...
        if (processUUIDs.size() > 0) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithInvolvedUser(username, processUUIDs);
          return result;
        } else {
          return 0;
        }
