...
public class WebGetProcessInstancesNumberWithActiveUser implements Command<Integer> {
  ...
  public WebGetProcessInstancesNumberWithActiveUser(final String userId, final Set<String> roleUUIDs,
    ...
    switch (processStartPolicy) {
      case ALLOW_BY_DEFAULT:
        ...
        if (processUUIDs != null && !processUUIDs.isEmpty()) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithActiveUserExcept(username, processUUIDs);
          return result;
        } else {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithActiveUser(username);
          return result;
        }
      ...
      case DENY_BY_DEFAULT:
        ...
        if (processUUIDs.size() > 0) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithActiveUser(username, processUUIDs);
          return result;
        } else {
          return 0;
        }
