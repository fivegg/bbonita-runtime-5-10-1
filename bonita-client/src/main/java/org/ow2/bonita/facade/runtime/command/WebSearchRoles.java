...
public class WebSearchRoles implements Command<WebSearchResult<Role>> {

  private static final long serialVersionUID = -5734790828777745064L;
   ...
   * @param pageSize
   */
  public WebSearchRoles(SearchQueryBuilder query, int fromIndex, int pageSize, boolean searchInHistory) {
  ...
  public WebSearchResult<Role> execute(Environment environment) throws Exception {
    ...
    if (nbOfPossibleResults > fromIndex) {
      final List<Role> items = queryRuntimeAPI.search(query, fromIndex, pageSize);
      if (items != null) {
        return new WebSearchResult<Role>(items, nbOfPossibleResults);
      }
    }
