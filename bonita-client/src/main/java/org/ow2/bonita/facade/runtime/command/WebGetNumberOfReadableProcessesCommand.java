 ...
 * 
 */
public class WebGetNumberOfReadableProcessesCommand implements Command<Integer> {
  ...
  public WebGetNumberOfReadableProcessesCommand(final boolean searchInHistory, final String userUUID,
  ...
  public Integer execute(final Environment environment) throws Exception {
    ...
    final List<Rule> applicableRules = managementAPI.getApplicableRules(RuleType.PROCESS_READ, userUUID, roleUUIDs,
    ...
    switch (processStartPolicy) {
      case ALLOW_BY_DEFAULT:
        // The exceptions are the processes the entity cannot read.
      ...
      case DENY_BY_DEFAULT:
        // The exceptions are the processes the entity can read.
