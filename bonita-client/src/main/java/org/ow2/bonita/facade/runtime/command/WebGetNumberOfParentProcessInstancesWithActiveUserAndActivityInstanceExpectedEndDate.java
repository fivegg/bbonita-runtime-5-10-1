 ...
 * 
 */
public class WebGetNumberOfParentProcessInstancesWithActiveUserAndActivityInstanceExpectedEndDate implements
    Command<Integer> {
  ...
  public WebGetNumberOfParentProcessInstancesWithActiveUserAndActivityInstanceExpectedEndDate(final String userId,
      final Set<String> roleUUIDs, final Set<String> groupUUIDs, final Set<String> membershipUUIDs,
      final String username, final int remainingdays) {
    this.userId = userId;
    ...
    switch (processStartPolicy) {
      case ALLOW_BY_DEFAULT:
        ...
        if (processUUIDs != null && !processUUIDs.isEmpty()) {
          ...
          result = queryRuntimeAPI
              .getNumberOfParentProcessInstancesWithActiveUserAndActivityInstanceExpectedEndDateExcept(username,
                  remainingdays, processUUIDs);
          return result;
        } else {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithActiveUserAndActivityInstanceExpectedEndDate(
              username, remainingdays);
          return result;
        }
      ...
      case DENY_BY_DEFAULT:
        ...
        if (processUUIDs.size() > 0) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithActiveUserAndActivityInstanceExpectedEndDate(
              username, remainingdays, processUUIDs);
          return result;
        } else {
          return 0;
        }
