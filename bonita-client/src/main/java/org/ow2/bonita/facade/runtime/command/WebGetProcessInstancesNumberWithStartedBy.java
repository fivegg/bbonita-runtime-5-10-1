...
public class WebGetProcessInstancesNumberWithStartedBy implements Command<Integer> {
  ...
  public WebGetProcessInstancesNumberWithStartedBy(final String userId, final Set<String> roleUUIDs,
    ...
    switch (processStartPolicy) {
      case ALLOW_BY_DEFAULT:
        ...
        if (processUUIDs != null && !processUUIDs.isEmpty()) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithStartedByExcept(username, processUUIDs);
          return result;
        } else {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithStartedBy(username);
          return result;
        }
      ...
      case DENY_BY_DEFAULT:
        ...
        if (processUUIDs.size() > 0) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithStartedBy(username, processUUIDs);
          return result;
        } else {
          return 0;
        }
