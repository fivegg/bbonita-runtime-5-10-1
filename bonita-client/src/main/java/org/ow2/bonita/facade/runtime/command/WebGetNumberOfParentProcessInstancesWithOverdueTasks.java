 ...
 * 
 */
public class WebGetNumberOfParentProcessInstancesWithOverdueTasks implements Command<Integer> {
  ...
  public WebGetNumberOfParentProcessInstancesWithOverdueTasks(final String userId, final Set<String> roleUUIDs,
      final Set<String> groupUUIDs, final Set<String> membershipUUIDs, final String username) {
    this.userId = userId;
    ...
    switch (processStartPolicy) {
      case ALLOW_BY_DEFAULT:
        ...
        if (processUUIDs != null && !processUUIDs.isEmpty()) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithOverdueTasksExcept(username, processUUIDs);
          return result;
        } else {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithOverdueTasks(username);
          return result;
        }
      ...
      case DENY_BY_DEFAULT:
        ...
        if (processUUIDs.size() > 0) {
          Integer result;
          result = queryRuntimeAPI.getNumberOfParentProcessInstancesWithOverdueTasks(username, processUUIDs);
          return result;
        } else {
          return 0;
        }
