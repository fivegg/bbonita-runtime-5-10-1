...
public class GetterBinding extends ElementBinding {
  ...
  public Object parse(Element getterElement, Parse parse, Parser parser) {
    ConnectorDescriptor descriptor = parse.findObject(ConnectorDescriptor.class);
    final String name = getChildTextContent(getterElement, "name");
    final Getter getter = new Getter(name);
    parse.pushObject(getter);
    parseElementList(getterElement, "metadata", "property", parse, parser);
    descriptor.addGetter(getter);
    return null;
  }
}
