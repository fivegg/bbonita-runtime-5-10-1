...
public class LocalStorageLoginModule implements LoginModule {
  ...
  public boolean login() throws LoginException {
    ...
    try {
      ...
      if (!callbacks.isEmpty()) {
        if (this.debug) {
          System.err.println("[" + LocalStorageLoginModule.class.getName() + "] login() - callback - step 2");
        }
      ...
      if (this.debug) {
        System.err.println("[" + LocalStorageLoginModule.class.getName() + "] login() - authenticating - step 3");
      }
      if (name != null) {
        this.id = name;
      } else {
      ...
      if (this.debug) {
        System.err.println("[" + LocalStorageLoginModule.class.getName() + "] login() - storing data - step 4");
      }
      this.sharedState.put(JAVAX_SECURITY_AUTH_LOGIN_NAME, name);
      this.sharedState.put(JAVAX_SECURITY_AUTH_LOGIN_PASSWORD, password);
      if (this.debug) {
        System.err.println("[" + LocalStorageLoginModule.class.getName() + "] login() - returning - step 5");
      }
      return true;
  ...
  public boolean logout() throws LoginException {
    if (this.id != null) {
      if (this.debug) {
        System.err.println("[" + LocalStorageLoginModule.class.getName() + "] logout() - removing principals");
      }
      ...
      for (final Principal p : principals) {
        if (p instanceof BonitaPrincipal) {
          if (this.debug) {
            System.err.println("[" + LocalStorageLoginModule.class.getName() + "] logout() - removing principal: " + p);
          }
